...
def create_backend(model, backend: str = "auto", logger=None) -> BaseBackend:
    ...
    # ---------- PyTorch ---------- #
    if backend in ("auto", "torch_impl"):
        if isinstance(model, Module):
            from floppy.backends.torch_impl.torch_backend import TorchBackend
            return TorchBackend(model, logger=logger)

    # ---------- Scikit-learn ---------- #
    if backend in ("auto", "sklearn_impl"):
        if isinstance(model, BaseEstimator):
            from floppy.backends.sklearn_impl.sklearn_backend import SklearnBackend
